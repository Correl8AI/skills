...
- For **agent identity and behavior** findings: recommend **system prompt** (and config if needed) changes that define agent role, behavioral rules, and constraints — not only tone
- For **user and channel context** findings: recommend passing user role, entry origin, or medium-specific context per turn, or tools to load them — based on what the product exposes at runtime, not catalog **User persona**
